...
later human-paired **Soul** phase (North Star "functionality first, surface soul later"; the impeccable UI gate and
human-in-the-room taste ride that pass). Where this spec says "branded," it means *uses the existing tokens*, not
...
7. **Gates green.** `pnpm typecheck`, `pnpm lint`, `pnpm test`, `pnpm build`, `pnpm secret-scan`, `ubs` (exit 0),
   and `npx impeccable detect src/` (UI touched) all pass.
