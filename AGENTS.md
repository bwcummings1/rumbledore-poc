...
- Content/feed: `content_item.league_id NULL` is central/open content; league home/feed queries still explicitly filter `league_id = current` inside `withLeagueContext()` so central news never becomes a league storyline by accident.
