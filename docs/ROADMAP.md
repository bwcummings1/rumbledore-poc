...
The durable phase plan toward the North Star (`docs/NORTH-STAR.md`). Live state lives in `docs/PROGRESS.md`; this file is the stable map of what each phase is and what still remains. (The old `IMPLEMENTATION_PLAN.md` loop-backlog is retired/historical — archived at `docs/archive/IMPLEMENTATION_PLAN.md`.) Timeline anchors: **~1 month to football buzz, ~3 months to the season** (season opens ~Sept 2026; this was written in the June 2026 offseason).
