...
2. **Substrate-B real source** choice; **hosted ESPN capture vendor** (**correction 2026-07-23, supersedes the
   2026-07-13 note below:** the guarded Browserbase adapter is now wired — `createBrowserbaseSession` in
   `src/onboarding/deps.ts`, mock-pinned; the `BrowserbaseSessionNotConfigured` stub no longer exists — and
   `BROWSERBASE_API_KEY`/`BROWSERBASE_PROJECT_ID` names are present in `.env.local`, values not inspected; a live
   smoke run remains owner-gated per `docs/runbooks/browserbase-live-smoke.md`); **email provider + domain** for
   the digest; **backup cron** installation (script exists, never scheduled).
