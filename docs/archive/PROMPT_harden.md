...
5. VALIDATE (mandatory backpressure): `pnpm typecheck` · `pnpm lint` · `pnpm test` · `pnpm build` · `ubs <changed files>`; if UI changed, follow `DESIGN.md` (AUSPEX — authoritative, near-pixel fidelity). Fix until ALL pass. Never weaken or skip a gate.
