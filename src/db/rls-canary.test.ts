...
beforeAll(async () => {
  ...
  [capabilityObservationA, capabilityObservationB] = await admin.db
    .insert(dataCapabilityObservations)
    .values([
      {
        availability: "full",
        dataClass: "teams",
        details: { marker, side: "a" },
        leagueId: leagueA,
        provider: "espn",
...
describe("two-league isolation under withLeagueContext", () => {
  ...
  it("sees no capability observations outside a league context", async () => {
    const rows = await canary.db
      .select()
      .from(dataCapabilityObservations)
      .where(
        inArray(dataCapabilityObservations.id, [
          capabilityObservationA.id,
          capabilityObservationB.id,
        ]),
      );
    expect(rows).toHaveLength(0);
  });
  ...
  it("scoped to league A, capability observations never reveal league B", async () => {
    const rows = await withLeagueContext(canary.db, leagueA, (tx) =>
      tx.select().from(dataCapabilityObservations),
    );
  ...
  it("rejects writing a league B capability observation from league A context", async () => {
    expect(
      await sqlstateOf(
        withLeagueContext(canary.db, leagueA, (tx) =>
          tx.insert(dataCapabilityObservations).values({
            availability: "none",
            dataClass: "rosters",
            leagueId: leagueB,
            provider: "espn",
  ...
  it("rejects direct capability observation mutation", async () => {
    expect(
      await sqlstateOf(
        withLeagueContext(canary.db, leagueA, (tx) =>
          tx
            .update(dataCapabilityObservations)
            .set({ rowCount: 11 })
            .where(
