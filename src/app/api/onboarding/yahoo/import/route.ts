...
import { importYahooDiscoveredLeague } from "@/onboarding/yahoo-service";
...
async function yahooImportDiscoveredPost(request: Request) {
  ...
  const result = await importYahooDiscoveredLeague(
...
export const POST = recordApiHandler(
  { method: "POST", route: "/api/onboarding/yahoo/import" },
  yahooImportDiscoveredPost,
);
