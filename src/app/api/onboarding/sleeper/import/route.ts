...
import { importSleeperDiscoveredLeague } from "@/onboarding/sleeper-service";
...
async function sleeperImportDiscoveredPost(request: Request) {
  ...
  const result = await importSleeperDiscoveredLeague(
...
export const POST = recordApiHandler(
  { method: "POST", route: "/api/onboarding/sleeper/import" },
  sleeperImportDiscoveredPost,
);
