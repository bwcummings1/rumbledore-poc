...
- [ ] Write the two-league RLS isolation canary integration test. (blocked-by: RLS helper — NOW UNBLOCKED. Must connect as a NON-superuser role: the compose user is a superuser and bypasses RLS even with FORCE; create a dedicated login role in the test (or an init script) with table GRANTs, then prove cross-league reads/writes fail under `withLeagueContext`.)
