...
- **REC-003 — DONE (2026-07-24, branch `ws/rec-followups-2026-07-24`).** After the owner directed completion, landed the low-risk, correctly-targeted fix: cap vitest worker concurrency **locally only** (`maxWorkers: process.env.CI ? undefined : "66%"` in `vitest.config.ts`) so the DB-heavy arena/bankroll/tailoring recomputes contend less on the single shared Postgres, while leaving CI at its default (its smaller runners already limit concurrency and its runs aren't flaky — a cap there would only slow them). Per-worker test databases remain the durable follow-up (removes shared-DB contention entirely; larger, changes test DB topology).
...
- **REC-006 — INVESTIGATED to a measurement-driven no-op (2026-07-24).** Timed `getLeagueCanonRecordsContext` on the real 95050 league (16 pushed seasons): **p50 988ms / p95 1277ms** — under the 2s interactive budget, no production load. Then *tried* the recommended cache (push-signature-keyed `composeCanonicalSnapshot`, provably staleness-safe given insert-only pushes, `structuredClone` for mutation safety) and **re-measured: it was slightly slower** (p50 ~1138ms). The measurement proved the ~1s is spent in the read-time *re-derivation*, not the snapshot compose the cache targeted — plus the signature query + a 3MB `structuredClone` added overhead. **Reverted the cache** rather than ship dead weight. Separately, the drift risk (two records-math implementations) is already guarded: `engine.test.ts` pins the write-time derivation and `records-catalog.test.ts` pins the read-time derivation to the same fixture-oracle numbers (e.g. `highest_single_week_score === 325`), so drift in either breaks CI. **Conclusion:** no code change is warranted — perf is under budget, the obvious cache is the wrong layer (measured), and drift is already test-guarded. The real perf lever (cache the *derived context*, or profile+optimize the derivation) is a profiled follow-up not justified by current evidence.
